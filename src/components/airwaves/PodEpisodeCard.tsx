...
export interface PodEpisodeRow {
  id: string;
  show_id: string;
  title: string;
...
export interface PodReactionRow {
  id: string;
  episode_id: string;
  user_id: string;
  emoji: string;
...
export default function PodEpisodeCard({
  episode,
  reactions,
  startAt,
  onSaveProgress,
}: {
  episode: PodEpisodeRow;
  reactions: PodReactionRow[];
  startAt?: number | null;
  onSaveProgress?: (episodeId: string, seconds: number) => void;
}) {
  const { user } = useAuth();
  ...
  const toggleReaction = useMutation({
    mutationFn: async (emoji: string) => {
      ...
      if (mine.has(emoji)) {
        ...
        const { error } = await supabase.from("pod_episode_reactions").insert({ episode_id: episode.id, user_id: user.id, emoji });
        if (error) throw error;
      }
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["pod-episode-reactions"] }),
    onError: (e) => toast.error(e instanceof Error ? e.message : "Could not react"),
  });

  const deleteEpisode = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("pod_episodes").delete().eq("id", episode.id);
      if (error) throw error;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["pod-episodes"] }),
    onError: (e) => toast.error(e instanceof Error ? e.message : "Could not delete"),
  });

  const { data: comments } = useQuery({
    queryKey: ["pod-episode-comments", episode.id],
    enabled: showComments,
    queryFn: async (): Promise<CommentRow[]> => {
      const { data, error } = await supabase
        .from("pod_episode_comments")
  ...
  const { data: commentCount } = useQuery({
    queryKey: ["pod-episode-comment-count", episode.id],
  ...
  const addComment = useMutation({
    mutationFn: async (body: string) => {
      if (!user) throw new Error("Not signed in");
      const { error } = await supabase.from("pod_episode_comments").insert({ episode_id: episode.id, user_id: user.id, body });
      if (error) throw error;
    },
    onSuccess: () => {
      setCommentBody("");
      queryClient.invalidateQueries({ queryKey: ["pod-episode-comments", episode.id] });
      queryClient.invalidateQueries({ queryKey: ["pod-episode-comment-count", episode.id] });
    },
  ...
  const deleteComment = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("pod_episode_comments").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["pod-episode-comments", episode.id] });
  ...
  const authorName = episode.author?.name ?? "Team member";
    ...
    <Card className="p-4 bg-card/60 border-border/30 rounded-2xl space-y-3">
      <div className="flex items-start gap-2">
        <div className="flex-1 min-w-0">
          <h3 className="font-display font-semibold text-foreground leading-snug">{episode.title}</h3>
          <div className="flex items-center gap-2 mt-1">
            <Avatar className="w-5 h-5 ring-1 ring-border/30">
              <AvatarImage src={episode.author?.avatar_url ?? undefined} />
            ...
            <span className="text-[11px] text-muted-foreground">· {relativeTime(episode.created_at)}</span>
          </div>
        </div>
        {episode.author_id === user?.id && (
          <button onClick={() => deleteEpisode.mutate()} className="text-muted-foreground hover:text-destructive transition-colors shrink-0" title="Delete episode">
      ...
      <AudioPlayer
        ...
        startAt={startAt}
        onProgress={(s) => onSaveProgress?.(episode.id, s)}
      />

      {episode.description && <p className="text-sm text-muted-foreground leading-snug whitespace-pre-line">{episode.description}</p>}
